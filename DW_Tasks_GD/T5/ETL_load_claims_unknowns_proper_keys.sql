...
BULK INSERT [dbo].[PartsCatalogueTemp] FROM 'C:\Users\laptop\Desktop\DW_generator\parts.csv'
WITH ( CODEPAGE = '65001'
      , FORMAT = 'CSV'
      , FIRSTROW = 2
      --, FIELDQUOTE = '\'
...
SELECT  Number_of_damaged_parts,
	    ...
	    Rear_bumper_cost,
	    Indemnity,
		ID_Assessment_Date = ISNULL(ADate.ID_Date, -1), -- surrogate ID from DT or UNKNOWN
		ID_Submission_Date = ISNULL(SDate.ID_Date, -1), -- ID from DT
		ID_Car = ISNULL(Car.ID_Car, -1), -- ID from DT
		ID_Assessor = ISNULL(Assessor.ID_Assessor, -1), -- ID from DT
		ID_Client = ISNULL(Client.ID_Client, -1) -- ID from DT
FROM --but JOIN DTs TBs via BKs
	[dbo].[vETLFactClaimData2] AS Claim
	LEFT JOIN [dbo].[DT_Date] AS ADate--BK Date
		ON CONVERT(VARCHAR(10), ADate.Date, 111) = CONVERT(VARCHAR(10), Claim.Assessment_date, 111)
	LEFT JOIN [dbo].[DT_Date] AS SDate--BK Date
		ON CONVERT(VARCHAR(10), SDate.Date, 111) = CONVERT(VARCHAR(10), Claim.Submission_date, 111)
	LEFT JOIN [dbo].[DT_Car] AS Car
		ON Car.VIN = Claim.VIN--BK VIN
		--ON Car.ID_Car = Claim.ID_Car
	LEFT JOIN [dbo].[DT_Assessor] AS Assessor--BK ID
		ON Assessor.ID_Assessor = Claim.ID_Assessor
	LEFT JOIN [dbo].[DT_Client] AS Client--BK PESEL
		ON Client.PESEL = Claim.PESEL AND
		   Claim.Submission_Date BETWEEN Client.Insertion_date AND ISNULL(Client.Deactivation_date, CURRENT_TIMESTAMP)
	WHERE Client.Is_Current = 1
	;
GO
