...
SELECT  Number_of_damaged_parts,
	    ...
	    Rear_bumper_cost,
	    Indemnity,
		ID_Assessment_Date = ISNULL(ADate.ID_Date, -1),
		ID_Submission_Date = ISNULL(SDate.ID_Date, -1),
		ID_Car = ISNULL(Claim.ID_Car, -1),
		ID_Assessor = ISNULL(Claim.ID_Assessor, -1),
		ID_Client = ISNULL(Client.ID_Client, -1)
FROM 
	[dbo].[vETLFactClaimData2] AS Claim
	LEFT JOIN [dbo].[DT_Date] AS ADate
		ON CONVERT(VARCHAR(10), ADate.Date, 111) = CONVERT(VARCHAR(10), Claim.Assessment_date, 111)
	LEFT JOIN [dbo].[DT_Date] AS SDate
	...
	LEFT JOIN [dbo].[DT_Assessor] AS Assessor
		ON Assessor.ID_Assessor = Claim.ID_Assessor
	LEFT JOIN [dbo].[DT_Client] AS Client
